...
export function ImageSlider({ images }: ImageSliderProps) {
    ...
    return (
        <section aria-label="Image Slider" style={{ width: "100%", height: "100%", position: "relative" }}>
            ...
            <button onClick={showPrevImage} className="img-slider-btn" style={{ left: 0 }} aria-label="View Previus Image">
            ...
            >
                {images.map((_, index) => (
                    <button
                        ...
                        onClick={() => setImageIndex(index)}>{index === imageIndex ? <CircleDot aria-hidden /> : <Circle />}
                    </button>
                ))}
            </div>
        </section >
